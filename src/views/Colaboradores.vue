...
const salvarColaborador = () => {
    ...
    if (colaboradorSelecionado.value.nome && colaboradorSelecionado.value.telefone && colaboradorSelecionado.value.dataNascimento && colaboradorSelecionado.value.pix.tipo && colaboradorSelecionado.value.pix.chave) {
        if (colaboradorSelecionado.value.id) {
            colaboradorService
                .atualizarColaborador(colaboradorSelecionado.value.id, colaboradorSelecionado.value)
                .then(() => {
            ...
            colaboradorService
                .salvarColaborador(colaboradorSelecionado.value)
                .then(() => {
...
const salvarVale = () => {
    ...
    if (valeSelecionado.value.valor) {
        ...
        if (valeSelecionado.value.id) {
            valeService
                .atualizarVale(valeSelecionado.value.id, valeSelecionado.value)
                .then(() => {
            ...
            valeService
                .salvarVale(valeSelecionado.value)
                .then(() => {
...
const deletarColaborador = () => {
    colaboradorService
        .deletarColaborador(colaboradorSelecionado.value.id)
        .then(() => {
...
const deletarVale = () => {
    valeService
        .deletarVale(valeSelecionado.value.id)
        .then(() => {
...
const deletarColaboradoresSelecionados = async () => {
    try {
        await Promise.all(
            colaboradoresSelecionados.value.map(async (colaborador) => {
...
const totalValesColaborador = () => {
    for (const colaborador of colaboradores.value) {
        colaboradorService
            .getValesColaborador(colaborador.id)
...
<template>
    ...
    <Dialog v-model:visible="colaboradorDialog" header="Detalhes do Colaborador" :modal="true" class="p-fluid max-w-min" :closable="false" :closeOnEscape="false">
        <div class="flex flex-column">
            <div class="flex flex-column">
                <div class="flex flex-row gap-3">
                    ...
                    <div class="field flex-1">
                        ...
                        <InputMask id="telefone" v-model="colaboradorSelecionado.telefone" required="true" mask="(99) 99999-9999" placeholder="(99) 99999-9999" autofocus :invalid="submitted && !colaboradorSelecionado.telefone" />
                ...
                <div class="flex flex-row gap-3">
                    ...
                    <div class="field flex-1">
                        ...
                        <Dropdown
                            id="tipo-colaborador"
                            v-model="colaboradorSelecionado.tipo"
                            :options="tiposColaboradores"
                            optionValue="tipo"
                            optionLabel="descricao"
                            placeholder="Selecione"
                            autofocus
                            :invalid="submitted && !colaboradorSelecionado.tipo"
                        />
            ...
            <div id="inputpix" class="flex flex-column gap-4 p-3 border-300 border-1 border-round max-w-max">
                <div class="flex flex-row gap-2">
                    <div v-for="tipoChavePix in tiposChavePix" v-bind:key="tipoChavePix.tipo">
                        <div
                            :class="colaboradorSelecionado.pix.tipo === tipoChavePix.tipo ? 'tipo-chave-button-selecionada' : ''"
                            class="tipo-chave-button flex flex-column justify-content-center align-content-center align-items-center border-1 border-round border-300 shadow-1 hover:shadow-3"
                            @click="setTipoPix(tipoChavePix.tipo)"
                        >
                ...
                <div v-if="colaboradorSelecionado.pix.tipo" class="fadein animation-duration-500">
                    <div v-if="colaboradorSelecionado.pix.tipo === 'CELULAR'">
                        ...
                        <InputMask id="chave-celular" v-model="colaboradorSelecionado.pix.chave" required="true" mask="(99) 99999-9999" placeholder="(99) 99999-9999" autofocus :invalid="submitted && !colaboradorSelecionado.pix.chave" />
                    ...
                    <div v-if="colaboradorSelecionado.pix.tipo === 'CPF'">
                        ...
                        <InputMask id="chave-cpf" v-model="colaboradorSelecionado.pix.chave" required="true" mask="999.999.999-99" placeholder="999.999.999-99" autofocus :invalid="submitted && !colaboradorSelecionado.pix.chave" />
                        <small class="p-error" v-if="submitted && !colaboradorSelecionado.pix.chave">Chave Pix é obrigatória.</small>
                    </div>

                    <div v-if="colaboradorSelecionado.pix.tipo === 'CNPJ'">
                        <label for="chave-cnpj">Informe o CNPJ</label>
                        <InputMask id="chave-cnpj" v-model="colaboradorSelecionado.pix.chave" required="true" mask="99.999.999/9999-99" placeholder="99.999.999/9999-99" autofocus :invalid="submitted && !colaboradorSelecionado.pix.chave" />
                        <small class="p-error" v-if="submitted && !colaboradorSelecionado.pix.chave">Chave Pix é obrigatória.</small>
                    </div>
    ...
    <Dialog v-model:visible="valeDialog" header="Lançamento de Vale" :modal="true" class="p-fluid max-w-min" :closable="false" :closeOnEscape="false">
        <div>
            <div class="field">
                <label for="nome">Colaborador</label>
                <p>
                    <b>{{ valeSelecionado.colaboradorNome }}</b>
                </p>
            </div>
    ...
    <Dialog v-model:visible="deleteColaboradorDialog" :style="{ width: '450px' }" header="Confirmação" :modal="true">
        <div class="confirmation-content">
            ...
            <span v-if="colaboradorSelecionado"
                >Tem certeza que deseja remover <b>{{ colaboradorSelecionado.nome }}</b
                >?</span
            >
        </div>
    ...
    <Dialog v-model:visible="deleteValeDialog" :style="{ width: '450px' }" header="Confirmação" :modal="true">
        <div class="confirmation-content">
            ...
            <span v-if="valeSelecionado"
                >Tem certeza que deseja remover o vale de <b>{{ colaboradorSelecionado.nome }}</b> no valor de <b>{{ util.formatCurrency(valeSelecionado.valor) }}</b
                >?</span
            >
        </div>
