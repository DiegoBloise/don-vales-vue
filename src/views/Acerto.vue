...
const buscarEntregadores = () => {
    ...
    if (dataInicio.value && dataFim.value) {
        entregadorService
            .getEntregadoresPorData(util.parseToLocalDate(dataInicio.value), util.parseToLocalDate(dataFim.value))
...
<template>
    <div class="card">
        ...
        <Dialog v-model:visible="acertoDialog" header="Acerto" :modal="true" class="p-fluid max-w-min fadein animation-duration-900" :closable="false" :closeOnEscape="false">
            ...
            <div class="flex flex-column fadein animation-duration-300 gap-4 m-2">
                <div class="flex flex-column gap-2">
                    <div class="flex flex-row gap-2 align-items-end justify-content-center ui-fluid">
                        <div class="field flex flex-column flex-grow-1">
                            ...
                            <Calendar class="w-12rem" id="data-inicio" v-model="dataInicio" showIcon :showOnFocus="false" dateFormat="dd/mm/yy" autofocus :invalid="submitted && !dataInicio" />
                            <small class="p-error" v-if="submitted && !dataInicio">Data início é obrigatório.</small>
                        </div>

                        <div class="field flex flex-column flex-grow-1">
                            <label for="data-fim">Data Fim:</label>
                            <Calendar class="w-12rem" id="data-fim" v-model="dataFim" showIcon :showOnFocus="false" dateFormat="dd/mm/yy" autofocus :invalid="submitted && !dataFim" />
                    ...
                    <div class="flex flex-row gap-2 justify-content-center align-items-end ui-fluid">
                        <div class="field flex flex-column flex-grow-1">
                            ...
                            <Dropdown
                                :disabled="entregadores == null || entregadores.length < 1"
                                id="entregadores"
                                v-model="entregadorSelecionado"
                                :options="entregadores"
                                optionValue="entregador"
                                optionLabel="nome"
                                placeholder="Selecione"
                                autofocus
                                :invalid="submitted && !entregadorSelecionado"
                            />
                ...
                <DataTable v-if="entregadorSelecionado.valorTotalComDesconto != null" ref="dt" :value="acertos" dataKey="id" :rows="10" removableSort striped-rows>
