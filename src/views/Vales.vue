...
const salvarVale = () => {
    ...
    if (valeSelecionado.value.valor && valeSelecionado.value.colaboradorId) {
    ...
    <Dialog v-model:visible="valeDialog" header="Lançamento de Vale" :modal="true" class="p-fluid max-w-min" :closable="false" :closeOnEscape="false">
        <div>
            <div class="field">
                ...
                <Dropdown
                    :disabled="valeSelecionado.id != null"
                    id="colaborador"
                    v-model="valeSelecionado.colaboradorId"
                    :options="colaboradores"
                    optionValue="id"
                    optionLabel="nome"
                    placeholder="Selecione"
                    autofocus
                    :invalid="submitted && !valeSelecionado.colaboradorId"
                />
    ...
    <Dialog v-if="isAdmin" v-model:visible="deleteValeDialog" :style="{ width: '450px' }" header="Confirmação" :modal="true">
        <div class="confirmation-content">
            ...
            <span v-if="valeSelecionado"
                >Tem certeza que deseja remover o vale de <b>{{ valeSelecionado.colaboradorNome }}</b> no valor de <b>{{ util.formatCurrency(valeSelecionado.valor) }}</b
                >?</span
            >
        </div>
